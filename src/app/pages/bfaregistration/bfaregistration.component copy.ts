...
fileBaseURL: any;
  ...
  // }
  const stepsNumber = Number(this.stepsNumber);
 console.log('Steps Number from Storage:', this.payment_status, stepsNumber);
if (this.payment_status === 'PAID' || stepsNumber > 3) {
  this.router.navigate(['/status']);
}
//this.termconditionsUrl= this.router.navigate(['/terms_conditions']);
 ...
 // ✅ Custom Latitude Validator
  ...
  //get cancelled_cheque() { return this.personalDetailsForm.get('cancelled_cheque'); }
  /** ----------------------- FORM INITIALIZATION ---------------------- **/
...
//  this.personalDetailsForm.get('cancelled_cheque_name')?.valueChanges.subscribe(value => {
//       const cancelled_cheque_name = this.personalDetailsForm.get('cancelled_cheque_name');
//       if(!cancelled_cheque_name?.value){
//          cancelled_cheque_name?.clearValidators();
//       }else {
//         cancelled_cheque_name?.setValidators([Validators.required]);
//       }

//       cancelled_cheque_name?.updateValueAndValidity();
  ...
  this.loading = true;
    const request = {
      ...
      profession_doc: this.personalDetailsForm.value['profession_doc_name']
       // cancelled_cheque: this.personalDetailsForm.value['cancelled_cheque_name']
    };
//console.log(request);
   this.bfaregistrationService.updatePost(request).subscribe({
      next: (res) => {
  ...
  this.bfaregistrationService.getData(request).subscribe({
    next: (response) => {
      ...
      this.personalDetailsForm.patchValue({
        ...
        //  cancelled_cheque_name:data.cancelled_cheque||'',
...
accountMatchValidation(event: Event): void {
  ...
  const request = {
    ifsc: ifscCode,
    account_no: confirmAccount
  };

  this.loading = true;
  this.submitted = true;

  this.bfaregistrationService.getBankData(request).subscribe({
    next: (res: any) => {
      this.loading = false;
      this.submitted = false;
    ...
    error: (error: any) => {
      ...
      this.loading = false;
      this.submitted = false;
      this.confirm_accountError = 'Verification failed';
    }
  });
}
